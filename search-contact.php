...
//On determine l'expression a qr
if(isset($_GET['q']))
{
        $rec = htmlentities($_GET['q']);
}
else
{
        $rec = 'php MYSQL';
}
//On determine le type de q
if(isset($_GET['type']))
{
...
$req .= ' order by id asc';
$requete = $db_connect->prepare($req);
$requete->execute();
//Le formulaire de q
?>
<form action="" method="get">
Expression &agrave; qr: <input type="text" name="q" value="<?php echo $rec; ?>" /><br />
Type de q: <input type="radio" name="type" value="un"<?php if($type==1){echo 'checked="checked"';} ?> /> Un des mots <input type="radio" name="type" value="tout"<?php if($type==2){echo 'checked="checked"';} ?> /> Tout les mots <input type="radio" name="type" value="exacte"<?php if($type==3){echo 'checked="checked"';} ?> /> Expression exacte<br />
...
while($dnn = $requete->fetch(PDO::FETCH_OBJ))
{
?>
        <tr>
...
if($surligner)//Si il faut surligner les mots, on les surligne
{
        ...
        {
                echo preg_replace('#('.str_replace(' ','|',preg_quote($rec)).')#i', '<strong>$1</strong>', $dnn['infos']);//On surligne les mots cles de la q
        }
}
else
{
